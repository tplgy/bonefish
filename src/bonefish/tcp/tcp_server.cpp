...
void tcp_server::on_connect(const std::shared_ptr<tcp_connection>& connection)
{
    std::cerr << "created connection" << std::endl;
    connection->set_close_handler(std::bind(&tcp_server::on_close,
            shared_from_this(), std::placeholders::_1));
    connection->set_fail_handler(std::bind(&tcp_server::on_fail,
            shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    connection->set_message_handler(std::bind(&tcp_server::on_message,
            shared_from_this(), std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    m_connections.insert(connection);
}

void tcp_server::on_close(const std::shared_ptr<tcp_connection>& connection)
{
...
void tcp_server::on_fail(const std::shared_ptr<tcp_connection>& connection, const char* reason)
{
    std::cerr << "connection failed: " << reason << std::endl;
    m_connections.erase(connection);
}

void tcp_server::on_message(const std::shared_ptr<tcp_connection>& connection,
